namespace HoneyWebPlatform.Services.Data
{
    ...
    public class UserService : IUserService
    {
        ...
        public async Task<bool> DeleteUserAndRelatedEntitiesAsync(Guid userId)
        {
            ...
            var beekeeper = await dbContext.Beekeepers.FirstOrDefaultAsync(b => b.UserId.ToString() == userId);
            if (beekeeper != null)
            {
            ...
            var userComments = await dbContext.Comments.Where(c => c.AuthorId.ToString() == userId).ToListAsync();
            dbContext.Comments.RemoveRange(userComments);

            var userOrders = await dbContext.Orders.Where(o => o.UserId.ToString() == userId).ToListAsync();
